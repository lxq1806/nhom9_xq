...
public class UserServiceImpl implements UserService {
	...
	@Override
	public boolean providerLogin(LoginDTO user) {
		User userCheck = this.findUserByUsername(user.getUsername());
		...
		if(BCrypt.checkpw(user.getPassword(), userCheck.getPassword()) && userCheck.getRole()==2) {
			...
			SessionUtilities.setProvider(ConvertUserToDto.convertUsertoDto(userCheck));

			log.info("userCheck:{}",SessionUtilities.getProvider().getUsername());
	...
	@Override
	public boolean checkProviderLogin() {
		return SessionUtilities.getProvider()!=null;
	}
